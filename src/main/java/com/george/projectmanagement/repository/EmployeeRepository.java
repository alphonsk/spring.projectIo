...
public interface EmployeeRepository extends CrudRepository<Employee, Long> {
    ...
    @Query(nativeQuery = true, value= "SELECT emp.first_name as firstName,  emp.last_name as lastName, emp.email as email, COUNT(pro.employee_id) as projectCount " +
            "FROM employee emp left join project_employee pro on pro.employee_id = emp.employee_id " +
            "GROUP BY  emp.first_name , emp.last_name , emp.email order by 3 desc")
